...
/* 超级块结构 - 必须与 mkfs.naive.c 完全一致 */
struct naive_super_block {
    __le32 magic;           /* 0x990717 */
    __le32 inode_total;     /* 128 */
    __le32 block_total;     /* 400 (200KB/512B) */
    __le32 inode_table_block_no; /* 4 */
    __le32 data_block_no;   /* 132 */
    __u8 padding[492];      /* 填充到512字节 */
};
...
static int naive_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    /* 调试：打印超级块前32字节 */
    printk(KERN_INFO "naivefs: superblock first 32 bytes:");
    for (i = 0; i < 32; i++) {
        if (i % 16 == 0) printk(KERN_CONT "\n");
        printk(KERN_CONT "%02x ", ((unsigned char *)nsb)[i]);
    }
    printk(KERN_CONT "\n");
    ...
    /* 打印超级块信息 */
    printk(KERN_INFO "naivefs: inode_total=%u, block_total=%u, inode_table=%u, data_blocks=%u\n",
           le32_to_cpu(nsb->inode_total),
           le32_to_cpu(nsb->block_total),
           le32_to_cpu(nsb->inode_table_block_no),
           le32_to_cpu(nsb->data_block_no));
