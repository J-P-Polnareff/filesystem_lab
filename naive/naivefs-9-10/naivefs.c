...
// 磁盘上的目录项结构（用于实现unlink）
struct naive_disk_dir_entry {
    __le32 inode;
    char name[NAIVE_MAX_FILENAME_LEN];
};

#define NAIVE_DIR_ENTRY_SIZE (sizeof(struct naive_disk_dir_entry))
...
// 释放文件占用的所有数据块
static void naive_free_file_blocks(struct super_block *sb, struct naive_disk_inode *disk_inode)
{
    struct naive_sb_info *sbi = NAIVE_SB(sb);
    int i;
    
    for (i = 0; i < NAIVE_DIRECT_BLOCKS; i++) {
        if (disk_inode->i_block[i] != 0) {
            int block_no = le32_to_cpu(disk_inode->i_block[i]);
            naive_clear_bit(block_no, sbi->block_bitmap);
            disk_inode->i_block[i] = 0;
            printk(KERN_INFO "naivefs: freed block %d\n", block_no);
        }
    }
...
static int naive_unlink(struct inode *dir, struct dentry *dentry)
{
    ...
    printk(KERN_INFO "naivefs: unlink called for %s (inode %lu)\n",
    ...
    // 如果链接数为0，释放资源
    if (inode->i_nlink == 0) {
        printk(KERN_INFO "naivefs: inode %lu has 0 links, freeing resources\n", inode->i_ino);
        
        // 释放文件占用的数据块
        naive_free_file_blocks(sb, disk_inode);
        ...
        // 释放inode位图
        naive_clear_bit(inode->i_ino - 1, sbi->inode_bitmap);
        mark_buffer_dirty(sbi->inode_bitmap_bh);
        
        // 标记inode缓冲区为脏
        mark_buffer_dirty(nii->inode_bh);
        
        printk(KERN_INFO "naivefs: inode %lu resources freed\n", inode->i_ino);
    }
    ...
    // 注意：我们这里没有实现从目录中删除目录项的实际逻辑
    // 因为这是一个简化的实现，实际文件系统需要遍历目录数据块
    // 找到对应的目录项并删除
    
    printk(KERN_INFO "naivefs: unlink completed for %s\n", dentry->d_name.name);
...
static int naive_rmdir(struct inode *dir, struct dentry *dentry)
{
    ...
    printk(KERN_INFO "naivefs: rmdir called for %s (inode %lu)\n",
           dentry->d_name.name, inode->i_ino);
    
    // 检查目录是否为空（简化的检查）
    if (inode->i_nlink > 2) {  // . 和 .. 算2个链接
        printk(KERN_ERR "naivefs: directory not empty\n");
        return -ENOTEMPTY;
    }
    
    // 调用unlink处理（因为目录删除逻辑类似）
...
static struct inode_operations naive_dir_inode_operations = {
    ...
    .unlink = naive_unlink,      // 新增：删除文件
    .rmdir = naive_rmdir,        // 新增：删除目录
};
...
MODULE_DESCRIPTION("Naive Filesystem with Read/Write/Delete Support");
